...
class IQLCritic(BaseCritic):
    ...
    def update_v(self, ob_no, ac_na):
        ...
        q_actions = q_values.gather(1, ac_na.unsqueeze(1)).squeeze(1)
        v_net_out = self.v_net(ob_no).squeeze()
        value_loss = self.expectile_loss(q_actions - v_net_out).mean()
        ### YOUR CODE END HERE ###
    ...
    def update_q(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        target = reward_n + self.gamma * (1-terminal_n) * next_v
        loss = self.mse_loss(q_actions, target)
        ### YOUR CODE END HERE ###
